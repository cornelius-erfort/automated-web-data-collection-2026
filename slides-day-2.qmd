...
## Web scraping skill: two-phase workflow

The `web-scraper` skill splits every scraping task into two phases — **skipping discovery produces fragile scrapers that break on the first site update.**

1.  **Discovery** — find the most stable + efficient data source
2.  **Script generation** — produce a single-file scraper ready to run on a schedule

------------------------------------------------------------------------

## Phase 1 — Discovery priority ladder

Work **top-down**; stop at the first level that works:
...
Output: a short report describing what was found and why it's the recommended source.

------------------------------------------------------------------------

## Phase 2 — Script generation

Produce a **single file** the user can run on a schedule (daily, monthly, …).

Tooling preference:

-   `httr` / `httpx` (pure HTTP) **>** headless browser — always prefer HTTP
-   `curl_cffi` (TLS fingerprinting workaround) as middle ground
-   `read_html_live()` / Playwright as **last resort**

Script requirements:

-   CLI args: `--output`, `--format` (jsonl/csv/parquet), `--date-range`, `--page-limit`, optional `--connection` for DB
-   Pagination handled automatically, polite delays built in
-   Progress logged to stderr; exit codes (`0` success, `1` partial, `2` failure)
-   `USER_AGENT` and `HEADERS` at the top for easy tuning
