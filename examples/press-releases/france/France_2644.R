...
# Set La France Insoumise specific variables
base_url <- "https://lafranceinsoumise.fr/category/actualites/communiques/page/"
party_id <- 2644
country_name <- "France"
party_name <- "La France Insoumise"
parlgov_party_id <- 2644
...
extract_press_links <- function(page_url) {
  tryCatch({
    ...
    links <- page %>%
      html_nodes(".elementor-heading-title.elementor-size-default a") %>%
      html_attr("href") %>%
      unique()
...
scrape_detailed_content <- function(url) {
  tryCatch({
    ...
    content <- page %>% html_node(".elementor-widget-container") %>% html_text(trim = TRUE)
...
# Main execution
message("Starting La France Insoumise press release scraper...")
