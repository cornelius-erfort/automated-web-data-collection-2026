...
# Set Green Party specific variables
base_url <- "https://gruene.ch/kategorie/medienmitteilungen/seite/"
party_id <- 141
...
# Function to fetch and process pagination data immediately
fetch_and_process_pagination_data <- function(base_url) {
  # Load last processed page number
  page_num_file <- "last_page.txt"
  if (file.exists(page_num_file)) {
    page_num <- as.integer(readLines(page_num_file))
  } else {
    page_num <- 1
  }
  ...
  while (consecutive_empty_pages < max_empty_pages) {
    ...
    if (length(links) > 0) {
      ...
      for (url in links) {
        if (url %in% results$url) {
          message("Skipping duplicate URL: ", url)
          next
        }
    ...
    writeLines(as.character(page_num), page_num_file)
    
    Sys.sleep(2)
    page_num <- page_num + 1
  }
  
  message("Completed processing all pages.")
}
...
scrape_detailed_content <- function(url) {
  tryCatch({
    ...
    (content <- page %>% html_nodes(".page__content > :not(header)") %>% html_text(trim = TRUE) %>% str_c(collapse = "\n"))
...
# Main execution
message("Starting Green Party press release scraper...")
fetch_and_process_pagination_data(base_url)
