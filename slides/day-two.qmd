...
-   Applied exercises
-   Syllabus: <https://cornelius-erfort.github.io/automated-web-data-collection-2026/index.html>
-   Cheat Sheet: <https://cornelius-erfort.github.io/automated-web-data-collection-2026/FUNCTIONS_AND_PACKAGES_CHEATSHEET.html>
...
## Agent-controlled browsers (also browser automation)

-   AI agents can drive a real browser (click/scroll/forms) and can be used **instead of RSelenium** in some workflows
...
-   **Tradeoffs:**
    ...
    -   Not fully deterministic → for research, convert the final approach into a reproducible script (`httr` / `rvest` / `RSelenium` / Playwright)
...
## Using AI assistants for web scraping (LLMs / agents)
...
## Agent vs script: choose by scale

-   **Small-scale / exploratory tasks:** agent-first can be great
    -   Let the agent control the browser, inspect the DOM/Network, click through flows, and produce a quick prototype
...
-   **Larger-scale / production tasks:** move to a script
    ...
    -   Better to convert the discovered logic into an R script (`httr`/API or stable selectors) and run reproducibly
-   Practical rule: use the agent to **discover and debug**, then use scripts to **scale and maintain**
...
-   Think of it as: **your lab’s standard operating procedure** encoded for the agent.
...
-   **Makes results more reliable**: always check `robots.txt`, rate limits, and data provenance.
-   **Makes code more consistent**: folder structure, naming, logging, retries, and saving outputs.
-   **Makes debugging faster**: standard steps (inspect → hypothesize → test → log → iterate).
...
## MCP (Model Context Protocol) in one slide
...
## When it doesn’t work: a debugging playbook

Most scraping tasks are simple when the page is stable: if the selector/API works, it works. The real skill is what to do **when it fails**.

-   **Step 0: confirm the basics**
    -   Is the page reachable? (status code, redirects, cookie wall)
    -   Is the content in the **HTML** you download, or only after JS runs?
-   **Step 1: inspect the DOM**
    -   Check if the element exists, and whether there are multiple similar elements
    -   Prefer selectors that are **stable** (semantic classes/ids) over positional ones (`nth-child`)
-   **Step 2: check the Network tab**
    -   Look for **XHR/fetch** calls that return JSON (often easiest for pagination/search)
    -   Reproduce the request with `httr` (headers, params, pagination cursor)
-   **Step 3: handle blocking and scale issues**
    -   Rate limits: slow down, add backoff, cache results
    -   Anti-bot (CAPTCHA/Cloudflare): consider official APIs / permissions; browser automation may still fail
-   **Step 4: make it reproducible**
    -   Save a small raw example (HTML/JSON), log errors, and write a minimal script that demonstrates the failure

How an assistant helps:

-   Turn “it broke” into a checklist (DOM vs Network vs blocking), propose fixes, and iterate quickly on small test pulls.
...
-   Choose a topic from the list or propose your own
-   Work can also include additional steps like (simple) text analysis, geocoding, or data visualization
